...
def split_into_bicodons(sequence: str, orf: int) -> List[str]:
    ...
    # Bicodon is 6 bases? Original code: j[i:i+2*CODON_SIZE] with step CODON_SIZE (3)
    # i.e. 1-6, 4-9, 7-12... overlapping processing?
    # Original: range(ORF, len(j), 3). Slice i:i+6. 
    # So yes, overlapping hexamers (codon pairs) stepping by 1 codon.
...
def calculate_conservation(sequences: List[str], orf: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Original code:
    # 1. conservation_matrix_reference_codons = [ref_codon, is_conserved_bool/count] (Actually it stored history counts then binary check)
    # 2. Sums up 'ConservationCount' for each unique codon type.
